...
RUN cp -f /app/nginx.conf /etc/nginx/nginx.template.conf \
    ...
    && FRONTEND_VERSION=$(curl -sL "https://api.github.com/repos/AlexanderHel/MoviePilot-Frontend-Engrisk/releases/latest" | jq -r .tag_name) \
    && curl -sL "https://github.com/AlexanderHel/MoviePilot-Frontend-Engrisk/releases/download/${FRONTEND_VERSION}/dist.zip" | busybox unzip -d / - \
    && mv /dist /public
EXPOSE 3000
VOLUME [ "/config" ]
